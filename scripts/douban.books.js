window["DOUBAN"]["BOOKS"]["FETCHER"] = function(twers, douban_books_callback) {
	this.twers = twers;
	this.fetch_books = function() {
		for (var i = 0; i < twers.length;i++) {
			var books_for_individual = new DOUBAN.BOOKS.DOMAIN.BOOKS()
			this.fetch_books_for(twers[i], 1, books_for_individual);
		}
	};
	this._parse = function(twer, book) {
		...
		return new DOUBAN.BOOKS.DOMAIN.BOOK(twer, image_url, douban_url)
	}
	this.fetch_books_for = function(twer, start_index, books_for_individual) {
		...
		DOUBAN.getUserCollection({
			...
			callback:function(books){
				...
				for(var idx = 0; idx < books.entry.length; idx++) {
					books_for_individual.add(doubanbooks._parse(twer, books.entry[idx]))
				}
				doubanbooks.fetch_books_for(twer, start_index + 50, books_for_individual)
			}
		});		
	}
}
